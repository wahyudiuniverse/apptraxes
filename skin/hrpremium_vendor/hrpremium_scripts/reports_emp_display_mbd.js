$(document).ready(function() {
    var xin_table = $('#xin_table').dataTable({
         "bDestroy": true,
         "ajax": {
             url : site_url+"reports/report_display_mbd/0/0/0/0/0/",
             type : 'GET'
         },
         dom: 'lBfrtip',
     ...
     jQuery.get(base_url+"/get_toko_by_employee/"+jQuery(this).val(), function(data, status){
       jQuery('#outlet_ajax').html(data);
     });
   });
     ...
     $("#attendance_datewise_report").submit(function(e){
         ...
         var xin_table2 = $('#xin_table').dataTable({
             "bDestroy": true,
             "ajax": {
                 url : site_url+"reports/report_display_mbd/"+emp_id+"/"+outlet_id+"/"+area_emp+"/"+start_date+"/"+end_date+"/",
