$(document).ready(function() {
   var xin_table = $('#xin_table').dataTable({
        "bDestroy": true,
		"ajax": {
            url : base_url+"/sub_projects_list/",
            type : 'GET'
        },
	...
	$('.edit-modal-data').on('show.bs.modal', function (event) {
		...
		var department_id = button.data('subproject_id');
		var modal = $(this);
	$.ajax({
		url : base_url+"/read/",
		type: "GET",
	...
	e.preventDefault();
		...
		$.ajax({
			type: "POST",
			url: base_url+'/add_subproject/',
			data: obj.serialize()+"&is_ajax=1&add_type=subproject&form="+action,
			cache: false,
			success: function (JSON) {
				if (JSON.error != '') {
					toastr.error(JSON.error);
