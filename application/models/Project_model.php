...
GROUP BY pros.project_id");
	...
	public function get_emp_by_project_id($project_id)
	{
		$query = $this->db->query("SELECT * FROM xin_user_mobile WHERE project_id = '$project_id'");
		return $query->result();
	}

	public function get_toko_by_filter($user_id)
	{
		$query = $this->db->query("SELECT DISTINCT (dismbd.customer_id), cus.customer_name 
