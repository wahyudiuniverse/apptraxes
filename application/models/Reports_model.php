...
WHERE employee_id = '99'");
	...
	public function filter_report_overtime($project_id, $sub_id, $area, $start_date, $end_date)
	{
		if ($sub_id == '0') {
			return $query = $this->db->query("
			SELECT 
    ...
    cust.no_contact, 
    cio.status_emp,
    DATE_FORMAT(cio.date_cio, '%Y-%m-%d') AS date_phone, 
    DATE_FORMAT(cio.datetimephone_in, '%H:%i:%s') AS time_in, 
    DATE_FORMAT(cio.datetimephone_out, '%H:%i:%s') AS time_out, 
    TIMEDIFF(cio.datetimephone_out, cio.datetimephone_in) AS timestay, 
    DATE(cio.datetimephone_in) AS extracted_date,
    TIME(cio.datetimephone_in) AS extracted_time,
	DATE(cio.datetimephone_out) AS extracted_date_out,
    TIME(cio.datetimephone_out) AS extracted_time_out
FROM `tx_cio` cio
LEFT JOIN xin_user_mobile userm ON userm.employee_id = cio.employee_id
LEFT JOIN xin_customer cust ON cust.customer_id = cio.customer_id
WHERE userm.project_id = '$project_id'
AND DATE_FORMAT(cio.date_cio, '%Y-%m-%d') BETWEEN '$start_date' AND '$end_date' 
AND cio.status_emp = 6
			");
		} else {
			return $query = $this->db->query("
...
AND cio.status_emp = 6;
	...
	public function filter_report_display_mbd_null () {
		return $query = $this->db->query("
		...
		LEFT JOIN xin_user_mobile userm ON userm.employee_id = dismbd.employee_id LIMIT 0
		");
	}
