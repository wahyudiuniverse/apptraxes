...
<?php 
	$hidden = array (
		'_method' => 'EDIT', 
		'_token' => $subproject_id, 
		'ext_name' => $fullname
	);
?>

<?php echo form_open('admin/project/update/'.$subproject_id, $attributes, $hidden);?>
<div class="modal-body">
  <div class="form-group">
    <label for="department-name" class="form-control-label"><?php echo $this->lang->line('xin_name');?>:</label>
    <input type="text" class="form-control" name="department_name" value="<?php echo '('.$subproject_id.') '.$fullname;?>"/>
  </div>
				...
				success: function (JSON) {
					if (JSON.error != '') {
						...
						var xin_table = $('#xin_table').dataTable({
							"bDestroy": true,
							"ajax": {
								url : "<?php echo htmlspecialchars(site_url("admin/project/sub_project_list")); ?>",
								type : 'GET'
							},
