...
//Process String
if( !function_exists('hrpremium_mail') ){

	function hrpremium_mail($from,$from_name,$to,$subject,$body){
...
// company info
if( !function_exists('hrpremium_company_name') ){

 function hrpremium_company_name(){
  $CI=& get_instance();
  $query =  $CI->db->query("SELECT company_name FROM xin_company_info")->row()->company_name;
  return $query;
 }
}
if( !function_exists('hrpremium_company_email') ){

 function hrpremium_company_email(){
  $CI=& get_instance();
